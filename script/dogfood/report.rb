...
out << <<~SCOPE
  ...
  - **観測エンジン本体(`Wrapper` / `Tracker` / `Session` / `TracePoint` 等)はこの
    ...
    (設計資料 §4.8 / `CLAUDE.md` ルール7、`gems/ethotrace/script/self_observe_engine.rb`)。#{GEM == "core" ? "結果は [self-observation-engine.md](./self-observation-engine.md) を参照。" : "結果は core gem の `docs/self-observation-engine.md` を参照。"}
