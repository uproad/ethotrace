...
> **パス正規化(`site.path` と §5 の io パス共通)**: 観測時に基準ディレクトリ
> (`Ethotrace.base_dir`、既定は観測対象プロジェクトのルート = 作業ディレクトリ)
> 配下のパスはそこからの相対パスへ畳む。`source_location` 等が返す絶対パスを
> そのまま記録すると観測環境のホームパスが混入する(§6)ため、core が出力前に
> 正規化する。基準の外(third-party gem / stdlib / tmp 等の外部資源)は相対化しても
> 可搬にならないため絶対のまま残す。
