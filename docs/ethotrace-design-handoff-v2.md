...
### 4.8 自己適用(self-hosting)と隔離戦略 — Ruby::Box

#### 問題

Ethotrace を Ethotrace 自身に適用すると、観測者と観測対象が**同一のクラス実体**を共有するため汚染が発生する(実装時に実際に発生した既知の問題)。Tracker を wrap すると、wrap された Tracker が記録のために Tracker 自身を呼ぶ構造的自己言及となり、再入ガード(§4.5)では防げない。フラグはメソッド単位の再帰を止める仕組みであり、「計装機構そのものが計装対象になる」事態には無力。

設計原則: **Ruby で書かれた Ruby 解析器は自身も解析可能であるべき(self-hosting)**。自己適用は CI の dogfooding ジョブとして常設する。

#### 解決: Ruby::Box(Ruby 4.0, experimental)による実体分離

Ruby 4.0(2025-12-25)で導入された Ruby Box は、`RUBY_BOX=1` 環境変数で有効化される実験的隔離機構(クラスは `Ruby::Box`)。box 内にロードされた定義(クラス/モジュール定義、モンキーパッチ、グローバル/クラス変数変更、ロードされたライブラリ)は box 内に隔離される。公式の想定ユースケースに「モンキーパッチを使うテストの隔離実行」が含まれる。

**アーキテクチャ: collector(root)/ probe(box内)分離**

```
root(メイン空間)                      box(Ruby::Box)
┌─────────────────────┐    参照注入    ┌──────────────────────┐
│ Ethotrace collector  │ ───────────▶ │ 解析対象               │
│  - Tracker           │              │  (自己適用時は          │
│  - 帰属機構          │ ◀─────────── │   box内Ethotraceコピー) │
│  - JSONLライター     │  イベント通知  │ Ethotrace probe(シム)  │
└─────────────────────┘              │  - box内クラスへのフック │
                                      └──────────────────────┘
```

- **collector** は root に置く。box 内の `Ethotrace::Tracker` と root の `Ethotrace::Tracker` は定数名が同じでも**別のクラス実体**なので、box 側を wrap しても root 側は無傷 → 汚染が構造的に解決。
- Box は box 内コードにコアクラスの独自ビューを与えるため、**root 側で `Time`/`ENV` 等に prepend したフックは box 内コードから見えない可能性が高い**。そのため box 内に薄い **probe** をロードし、box 内のクラス実体に対してフックを張り、root から注入された collector オブジェクト参照経由でイベントを通知する(Box は Ractor と異なり同一ヒープ共有のため、オブジェクト参照のブリッジが原理的に可能)。
- probe/collector 分離は OpenTelemetry の agent/collector と同型。Box なし環境では「probe と collector が同一空間にいる」形に自然に縮退する。

#### core の依存方針

**core は Ruby::Box に依存しない。** 隔離は差し替え可能な isolation strategy として実装する:

| 戦略 | 条件 | 用途 |
|---|---|---|
| `NullIsolation` | 既定 | 通常の解析(対象 ≠ Ethotrace)。probe = collector 同一空間 |
| `BoxIsolation` | Ruby >= 4.0 かつ `RUBY_BOX=1` | 自己適用、モンキーパッチの激しい対象の解析 |
| `Stage0Bootstrap` | Ruby 3.x フォールバック | リリース済み gem(stage0)で開発ツリー(stage1)を解析するコンパイラ式ブートストラップ。stage0 を別プロセスまたはリネーム済み名前空間でロード |

#### Ruby::Box の既知の制約(2026-06 時点の調査)

1. **experimental**。将来挙動が変わりうる。CI では Ruby 4.0 + `RUBY_BOX=1` を独立ジョブとして隔離し、失敗を必須チェックにしない(allow-failure)。
2. **セキュリティサンドボックスではない**(名前空間の隔離のみ)。Ethotrace の用途(観測)には十分。
3. `RUBY_BOX=1` 下で**ネイティブ拡張 gem のインストールが失敗しうる**。回避策: フラグなしで `bundle install` し、実行時のみ `RUBY_BOX=1` を付与。
4. **ActiveSupport core_ext の一部に互換性問題** → **Rails アダプタと BoxIsolation の併用は当面サポート外**と明記する。
5. ENV の実値・ファイルディスクリプタ・GC・シグナル等のプロセスグローバル資源は box 間で共有される(隔離されるのは定義のみ)。

#### 実装前に確定すべき検証実験(M5 の最初のタスク。M2 時点での先行実施を推奨)

- **E1(最重要)**: root で enable した TracePoint は box 内の `:call` イベントを拾うか。拾う場合 `tp.self` の同一性・追跡テーブルとの突き合わせは機能するか。拾わない場合、TracePoint も probe 側(box 内)で enable する設計に変更。
- **E2**: root から box 内の定数・クラス実体へアクセスする公式 API(`box::Const` 形式 / `box.eval` / `box.require`)の確定。
- **E3**: root のオブジェクト(collector)を box 内に渡してメソッド呼び出しした際、どちらの空間の定義で実行されるか(メソッド解決の所属空間)。Thread-local 再入ガードは同一スレッドなら box を跨いでも有効かの確認。
- **E4**: `box内のEthotrace::Tracker != rootのEthotrace::Tracker` の実体分離をテストで固定化。
- **E5**: box 内コードの `Time.now` / `ENV[]` が box 側ビューのクラスを解決することの確認(= probe 側フックの必要性の裏取り)。
- **E6**: box 1個あたりのメモリ・起動オーバーヘッドの計測。
...
13. **自己適用は専用の隔離戦略が必須**(§4.8)。再入ガードだけでは「計装機構そのものが計装対象になる」自己言及を防げない(実装時に実際に発生)。core の計装パスに「自身の名前空間を計装対象から除外する deny-list」を defense-in-depth として常設しつつ、本解決は BoxIsolation / Stage0Bootstrap で行う。
14. Ruby::Box は experimental(§4.8)。`RUBY_BOX=1` 下のネイティブ拡張インストール問題、ActiveSupport core_ext 互換性問題に注意。**Rails アダプタ + BoxIsolation の併用は当面サポート外**。

---

## 9. 実装ロードマップ

各マイルストーンは「動くもの + テスト」で完結させる。TDD 推奨。

順序の意図: **M4 完了時点で「RSpec でテストされている Ethotrace 自身」が検査可能な最初の実用対象になる**ため、直後の M5 で自己適用を達成する。続く M6 で MCP を実装することで、**以降の開発は Claude Code が MCP 経由で Ethotrace の自己観測結果(各メソッドの規約・例外・Requirements)を参照しながら進められる**——ツールが自分自身の開発を支援するブートストラップループに入る。Rails アダプタ以降はこのループの恩恵を受けて実装する。
...
- **M2: Requirements チャネル + stdlib アダプタ**
  ...
  - 推奨: この時点で §4.8 の検証実験 E1〜E6 を先行実施し、probe/collector 分離を見越した構造にしておく(M5 のリファクタリングコスト削減)。
- **M3: 引数プロトコル観測**
...
- **M5: 自己適用(self-hosting)と隔離戦略**
  - §4.8 の検証実験 E1〜E6(M2 で未実施なら最初に実施)。結果を `docs/box-semantics.md` に記録(Box の挙動は experimental のため、検証した Ruby バージョンを明記)。
  - isolation strategy 抽象(`NullIsolation` / `BoxIsolation` / `Stage0Bootstrap`)の導入。collector / probe の分離リファクタリング。
  - 自己適用 E2E: 「Ethotrace のテストスイート(M4 で RSpec 接続済み)を Ethotrace で観測し、Ethotrace 自身の観測レコードが得られる」ことをゴールとする。
  - CI: Ruby 4.0 + `RUBY_BOX=1` の dogfooding ジョブを追加(experimental のため allow-failure 設定)。
- **M6: MCP サーバ(`ethotrace-mcp`)— ブートストラップループの起点**
  - 「このメソッドの引数0は何を要求するか」「R=∅(純粋)なメソッド一覧」「`time.read`/`random` を含む flaky 容疑メソッド一覧」「このメソッドが投げうる例外」等のクエリをツールとして公開。
  - 第一のユーザーは **Ethotrace を開発する Claude Code 自身**。M5 の自己観測データ(`ethotrace/observations.json` 等)を読む構成で接続し、以降の開発ワークフロー(テスト実行 → 自己観測更新 → MCP 経由で参照)を `CLAUDE.md` に手順として記載する。
  - クエリ設計はこの実体験(自分で使って不足を感じた点)を仕様にフィードバックする。
- **M7: Rails アダプタ**
  - Railtie、Notifications 購読(`db.query`)、Zeitwerk ロード時計装、Net::HTTP フック。
  - サンプル Rails アプリ(リポジトリ内 `examples/rails-app`)で E2E。
  - 注意: BoxIsolation との併用は当面サポート外(§8 項目14)。Rails 解析は `NullIsolation` で行う。
- **M8: RBS 投影(`ethotrace-rbs`)**
  - protocol → RBS `interface` 投影(例外・Requirements は RBS に存在しないため欠落をコメントで明記)。
...
- 自己適用(M5 以降): Ethotrace 自身のテストスイートを観測対象とする dogfooding を CI に常設。隔離戦略のリグレッション検出を兼ねる。
...
8. Ruby::Box の詳細セマンティクス(TracePoint の box 境界挙動ほか)— §4.8 の検証実験 E1〜E6 の結果待ち。設計判断(probe 側 TracePoint 化など)はその結果に従う。
...
- OpenTelemetry のスパン階層 — エフェクトスパン設計の参照モデル。collector/probe 分離(§4.8)も agent/collector 構成と同型。
- Effect-TS の `Effect<Success, Error, Requirements>` — 三チャネルモデルの着想元。
- **Ruby::Box** — Ruby 4.0(2025-12-25)の experimental 隔離機構。`RUBY_BOX=1` で有効化。公式リリースノート: https://www.ruby-lang.org/en/news/2025/12/25/ruby-4-0-0-released/ — 自己適用の隔離戦略(§4.8)の基盤。
...
- **M6 完了後は、開発サイクルに自己観測を組み込むこと**: テスト実行 → 自己観測データ更新(`ethotrace merge`)→ 実装・リファクタリング前に `ethotrace-mcp` で対象メソッドの規約・例外・Requirements を確認、というループを標準ワークフローとする。この手順を `CLAUDE.md` に明記し、MCP クエリで不足を感じた点は `ethotrace-mcp` の仕様改善としてフィードバックする。