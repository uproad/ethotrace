...
```bash
cd gems/ethotrace-mcp
ETHOTRACE_DOGFOOD_SESSION=mcp ETHOTRACE_DOGFOOD_OUT=tmp/dogfood \
  bundle exec rspec -r ../../script/dogfood/observe.rb -I spec \
  spec/ethotrace/mcp_spec.rb spec/ethotrace/mcp
...
ETHOTRACE_SELF_GEM=mcp bundle exec ruby ../../script/dogfood/report.rb
```
...
|---|---|---|---|---|
| `MCP::CLI.start` | Integer | `io.read`, `time.read` | — | 3 |
...
| `MCP::Server.build` | MCP::Server | `io.read` | — | 12 |
| `MCP::Stdio.run` | StringIO | `time.read` | — | 5 |
| `MCP::Tools.exceptions_of` | NilClass | — | — | 12 |
| `MCP::Tools.flaky_suspects` | NilClass | — | — | 12 |
| `MCP::Tools.install` | NilClass | `io.read` | — | 12 |
| `MCP::Tools.list_methods` | NilClass | — | — | 12 |
| `MCP::Tools.lookup_method` | NilClass | `io.read` | — | 12 |
| `MCP::Tools.method_args` | Hash | — | — | 3 |
| `MCP::Tools.methods_raising` | NilClass | — | — | 12 |
| `MCP::Tools.param_protocol` | NilClass | — | — | 12 |
| `MCP::Tools.pure_methods` | NilClass | — | — | 12 |
| `MCP::Tools.requiring` | NilClass | — | — | 12 |
...
- **純メソッド(R=∅ かつ 伝播例外=∅): 18 / 23。** 観測された範囲では副作用も伝播例外も持たない。
- **非決定性の疑い(`time.read` / `random.read`): 2 件。** `MCP::CLI.start`, `MCP::Stdio.run`。
- **Requirements を記録したメソッド: 5 件。** 観測された語彙: `io.read` / `time.read`。
- **伝播例外: 0 件。** テストはおおむね正常系を通すため、例外パスは未到達。これは observed contract が下限であることの具体例。
- **引数プロトコルの収穫の例**: `MCP::Stdio.run` は `server` → `accepts_server_context?`, `add_instrumentation_data`, `call_tool`, `call_tool_with_args`, `cursor_from`, `dispatch_optional_context_handler`, `handle_json`, `handle_request`, `handler_declares_server_context?`, `instrument_call`, `list_tools`, `paginate`, `server_context_with_meta`, `validate_tool_call_result!` を記録した。これは Ethotrace が公称型ではなく「実際に要求された振る舞い(プロトコル)」を捉える狙いそのもの。
